...
// 解决ssr报错问题
export function changeThemeColor(Color: any, theme: string, mode: 'light' | 'dark') {
  const { colors: newPalette, primary: brandColorIndex } = Color.getColorGradations({
    colors: [theme],
    step: 10,
    remainInput: false // 是否保留输入 不保留会矫正不合适的主题色
  })[0]
  const newColorMap: any = generateColorMap(Color, theme!, newPalette, mode, brandColorIndex)
